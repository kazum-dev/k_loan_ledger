        ...
        with open('customers.csv',mode='r',newline='',encoding='utf-8') as file:
            ...
            if not customers or all(not row for row in customers):
                ...
                for row in customers:
                    if row:
                        print(f"ID: {row['customer_id']}, 名前: {row['customer_name']}, 貸付上限額: {int(row['credit_limit']):,}円")
    except FileNotFoundError:
        print("顧客データが見つかりません。")
    ...
    try:
        with open('customers.csv', mode='r', newline='', encoding='utf-8') as file:
            ...
            if matches:
                ...
                for row in matches:
                    print(f"ID:{row['customer_id']}, 名前:{row['customer_name']}, 上限額:{int(row['credit_limit']):,}円")
            else:
...
def get_all_customer_ids():
    ...
    try:
        with open('customers.csv', mode='r', newline='', encoding='utf-8') as file:
            ...
            for row in reader:
                if row:
                    customer_ids.append(row[0].strip()) #文字列として扱う（intは使わない）
    except FileNotFoundError:
        print("顧客データが見つかりません。")
