...
def is_over_repayment(loans_file, repayments_file, loan_id, repayment_amount):
    ...
    if total_repaid + repayment_amount > repayment_expected:
        print(
            f"❌ 返済額が予定額を超えています。現在の累計返済額：{total_repaid}円 / 予定：{repayment_expected}円"
        )
        return False
...
def display_unpaid_loans(
    customer_id, loan_file="loan_v3.csv", repayment_file="repayments.csv"
):
    try:
        # 貸付データを読み込む
                ...
                if due_date_str:
                    try:
                        ...
                        if due_date < today:
                            ...
                            status += (
                                f"|延滞日数：{days_late}日|延滞手数料：¥{late_fee:,}"
                            )
                    except ValueError:
                        status = "⚠期日形式エラー"
    ...
    try:
        ...
        if overdue_loans:
            ...
            for loan in overdue_loans:
                ...
                # 延滞情報を表示
                print(
                    f"{loan_date}｜{amount_str}｜返済期日：{due_date_str}｜延滞：{days_late}日｜手数料：¥{late_fee:,}"
                )
        else:
            # 延滞がない場合のメッセージ
