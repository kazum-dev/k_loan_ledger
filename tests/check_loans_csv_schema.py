...
with p.open("r", encoding="utf-8-sig", newline="") as f:
    ...
    for i, row in enumerate(r, start=2):  # 1行目ヘッダ、データは2行目〜
        # 1) 必須列の存在と空値
        required = ["loan_id","customer_id","loan_amount","loan_date","repayment_expected",
                    "repayment_method","grace_period_days","late_fee_rate_percent","late_base_amount"]
        ...
        # 3) 数値列の型チェック
        def is_int_like(x): 
            try: int(float(x)); return True
            except: return False
        def is_float_like(x):
            try: float(x); return True
            except: return False
        bad_types = []
        if not is_int_like(row.get("grace_period_days","0")): bad_types.append("grace_period_days")
        if not is_float_like(row.get("late_fee_rate_percent","")): bad_types.append("late_fee_rate_percent")
        if not is_int_like(row.get("late_base_amount","")): bad_types.append("late_base_amount")
