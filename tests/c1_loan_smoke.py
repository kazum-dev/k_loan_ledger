...
print("case1:", calc_overdue_days(
    today=date(2025, 9, 10),
    due_date_str="2025-09-01",
    grace_period_days=3
))

print("case2（当日以内）:", calc_overdue_days(
    today=date(2025, 9, 10),
    due_date_str="2025-09-01",
    grace_period_days=9
))
...
if Path(repay_csv).exists():
    with open(repay_csv, "r", encoding="utf-8-sig", newline="") as f:
        ...
        for row in reader:
            ...
            if row.get("loan_id") or row.get("LOAN_ID") or next(iter(row.keys()),"").lower().startswith("loan"):
                # できるだけ素直に拾う
                first_id = (row.get("loan_id")
                            or row.get("LOAN_ID")
                            or row.get(next(k for k in row.keys() if k.lower().startswith("loan"))))
                break
...
print("overpay? demo ->", is_over_repayment(paths["loans_csv"], paths["repayments_csv"], "L20250801-001", 5000))
print()
