...
主要機能の回帰確認には pytest を使用しています。

開発用依存関係は以下でインストールできます。
...
テスト実行時は、本番環境の Neon PostgreSQL や通常のローカル SQLite を使用せず、テスト専用の一時 SQLite データベースを使用します。

現在、自動テストでは主に以下を確認しています。

* ログイン・ログアウト・未ログイン時のアクセス制御
* ユーザーごとのデータ分離
* 顧客登録と入力値チェック
* 貸付登録と入力値チェック
* 返済登録・過剰返済・延滞手数料処理
